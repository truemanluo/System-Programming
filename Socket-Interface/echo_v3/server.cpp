 ...
 * @param  {int} fd       : 
 * @param  {void*} buf    : 
 * @param  {size_t} count : 每次读取的固定字节数
 * @return {ssize_t}      : 成功读取的字节数
 */
ssize_t readn(int fd, void *buf, size_t count) {
    size_t nleft = count;
    ssize_t nread; // 已经读取的字节数
    char *usrbuf = (char*)buf;
    while (nleft > 0) {
        if ((nread = read(fd, usrbuf, nleft)) < 0) {
            if (errno == EINTR) continue; // 被中断时继续读取
            return -1;
        }
        else if (nread == 0) { // 刚好读完了
            break;
        }
        nleft -= nread;
        usrbuf += nread;
